...
# Headless-safe (optional but recommended for CI/servers)
os.environ.setdefault("MPLBACKEND", "Agg")
import matplotlib.pyplot as plt

# ----------------------------
# Resolve repo root robustly (no CWD dependence)
# ----------------------------
...
# ----------------------------
# Paths aligned with your actual assets layout (per screenshot)
#   assets/
#     clim_vec.npy
#     norm_stats.json
#     reports/  (outputs)
...
def _find_ckpt(weights_dir: Path = WEIGHTS_DIR) -> str:
    ...
    Returns:
      str: path to the checkpoint prefix (without ".index").
    """
    ...
    idx = sorted(weights_dir.glob("*.ckpt.index"))
    if idx:
        return str(idx[0]).replace(".index", "")
...
def _plot_year_all_models(
    X, Y, tgt_dates, model, st, clim_vec, year: int, lookback: int = 14, models_to_plot=None
):
    """
    ...
    out_png = REPORTS_DIR / f"fig_year_{year}_subset.png"
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=None,
        help="Plot a full-year Day-7 curve (e.g., 2024) and save to assets/reports/fig_year_<year>_subset.png",
    )
    ap.add_argument(
        "--plot_models",
        type=str,
    ...
    args = ap.parse_args()
    models_to_plot = {s.strip() for s in args.plot_models.split(",") if s.strip()}

    # --- sanity check for required artifacts ---
    ...
    runner = TenYearUnifiedRunner(str(CSV_DIR), seq_length=SEQ_LENGTH, pred_length=PRED_LENGTH)
    ...
    with open(str(NORM_PATH), "r", encoding="utf-8") as f:
        st = json.load(f)
    ...
    # 5) Save the table
    REPORTS_DIR.mkdir(parents=True, exist_ok=True)
    out_csv = REPORTS_DIR / "metrics.csv"
    df.to_csv(str(out_csv), index=False)
    print(f"Saved metrics to: {out_csv}")

    # 6) Optional plotting: selected 60-day window
    if args.plot_window != "none":
        ...
        if len(idx) > 0:
            ...
            subset_txt = ", ".join(sorted(models_to_plot)) if models_to_plot else "all"
            plt.title(f"Window={key} — Day-7 absolute WL (subset: {subset_txt})")
            plt.xlabel("Date")
            plt.ylabel("Water Level (m)")
            plt.grid(True, alpha=0.3)
            plt.legend()
            ...
            fig_path = REPORTS_DIR / f"fig_{key}_subset.png"
